...
//----------------------------------------------------------------------------
static UINT32 SquareRootOf(UINT64 lValue)
{
  UINT32 nResult = 1;
  UINT32 nTestBit;
  UINT64 lTemp;
  ...
  while (nTestBit >>= 1)
  {
    ...
    if (lValue < (unsigned long long)nResult * nResult)
      nResult &= ~nTestBit;
  }
  return nResult;
}
...
static INT64 DecelRamp(tTRAJECTORY* pTrajectory, UINT32 nDistance)
{
  UINT64 lTemp;
  ...
  if (lTemp >= 0x8000000000000000)   //(lTemp >= 0x80000000)
    return 9223372036854775807;  //INT32_MAX;
  // Times two is safe:
  lTemp <<= 1;
...
{
    INT64 lDecelRamp;
    UINT32 nPosition = pTrajectory->lPosition >> 16;
    UINT64 lMaxVelocity = ((UINT64)(pTrajectory->nMaxVelocity) << 16) / TRAJECTORY_FREQ;
