...
#define STEADYVEL       300000 //20000 //50  //5000  //360000000
#define ACCEL           30000  //2   //2000  //250000000
#define DECEL           30000  //2 //2000  //250000000
...
float   lHomeinch = -1;  // received   //offset from negative over travel limit up to 69mm for now 70mm per rev
float   lMoveMM = 200; //  received  distance back and forth to cut
float   lMoveinch = -1.0;  //-4.7124; //  received  distance back and forth to cut 4.7124 inches per rev of motor
...
//static void ESTOP_User_Handler(CN_PIN I_Estop_PIN, uintptr_t context)
{      
...
static void STOP_User_Handler(CN_PIN I_Stop, uintptr_t context)
{      
       if(I_Stop_Get() == SWITCH_ON_STATE){
           Stop = true;
           ControlledStop = true;
        }
       else{
...
static void START_User_Handler(CN_PIN I_Start, uintptr_t context)
{      
       if(I_Start_Get() == SWITCH_ON_STATE){
           Start = false;
        }
       else{
           Start = true;
...
void VescCanFeedRoller()
{
   ...
   //switch(iVescCase)   
    switch(iVescCase)
    {
        case 0:
            ...
            GPIO_PinInterruptEnable(CN16_PIN);  //CN16_PIN
            GPIO_PinInterruptCallbackRegister(CN13_PIN, STOP_User_Handler, 0);
            GPIO_PinInterruptEnable(CN13_PIN);
            GPIO_PinInterruptCallbackRegister(CN14_PIN, START_User_Handler, 0);
           ...
           // lMoveDistance = (lMoveMM/70)*360;   //1.5 inches diameter 4.7124 inches per rev
            lMoveDistance =  (lMoveinch/4.652)*360;  //4.7124
            lHomeOffset = (lHomeinch/4.652)*360; 
            PartCount = 0;
            PartsLeft = 0;
            sendPos = 0;
            direction = false;
  ...
  //          iVescCase = iVescCase + 1*((Button1Trigger()) || (Start));
            iVescCase = 60;
        break;
        ...
        case 60:
            ...
            //When Button is pressed start moving the motor or a delay then start
             iVescCase = iVescCase + 2*(((Button1Trigger())||(Start)||(Delay >= 200000)));
        //     iVescCase = iVescCase + 2*(((Button1Trigger())||(Start)));
        break;
        ...
        case 62:
            ...
            PartsTotal++;
            Delay = 0;
            // print to serial the pos and start status
...
{
    uint32_t ID =  (CMD_ID_STATUS << 8) + unit_id;  //unit_id;  //unit_id; // 2 for saw 1 for FeedRoller
...
void SetDuty(int unit_id,float Duty)  // 001 for id 1
{
...
void SetPos(int unit_id, int32_t Pos)  // hex 401 for id 1
{
