...
int32_t lHomeOffset = 0;  //63  319;
int32_t lMoveDistance = 90;  //;
int32_t lMoveIncrement = 30;  //;
int32_t Pos = 0;
int32_t Rpm = 0;
int AtSpeed = 0;
int AtPos   = 0;
...
int main ( void )
{
    ...
    while ( true )
    {
        ...
        //iMoveQuotient = (lMoveDistance / 60);
       // iMoveRemainder = (lMoveDistance % 60);
        ...
        case 50:
            ...
            if(VescFlags.bit.ValidMsgs)
            {
               if (!CAN1_TxFIFOIsFull(0) )
              {
                ...
                Delay++;
            if(Delay >= 1000000)  //100,000  looks like one second
            {
                Delay = 0;
                CutPos = 0;
                //Goto next state
            ...
            if (!CAN1_TxFIFOIsFull(0) )
            {
                ...
                if(CutPos >= iMoveQuotient * lMoveIncrement)
                  //if(CutPos >= 120)
                     {
                        CutPos = 0;
                        //sprintf(VescRx_msg,"iMoveQuotient\r\n");
                        //UART1_Write(&VescRx_msg, sizeof(VescRx_msg)); 
                        iVescCase = 69;
                        //break;
                     }                                  
